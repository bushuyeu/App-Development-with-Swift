 ...
 */
func add(_ firstInput: Int?,and secondInput: Int?) -> Int? {
    guard let firstInput = firstInput, let secondInput = secondInput else { return nil}
    let add = firstInput + secondInput
    return add
}
...
let user = createUser()
if let user = user {
    print("\(user.age), \(user.firstName) \(user.lastName)")
}
